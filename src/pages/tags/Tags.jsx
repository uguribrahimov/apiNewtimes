...
const Tags = () => {
  ...
  const getTags = async () => {
    try {
      const response = await axiosInstance.get('dashboard/tags/index');
  ...
  const addTags = async (values) => {
    await axiosInstance.post(`dashboard/tags/store`, values);
    toggleModal();
    getTags();
    reset({
      name: "",
  ...
  const deleteTags = async (id) => {
    await axiosInstance.delete(`dashboard/tags/delete/${id}`);
    getTags();
  };
